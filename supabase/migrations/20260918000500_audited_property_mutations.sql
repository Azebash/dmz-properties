create function public.save_property(p_payload jsonb)
returns uuid
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  if property_id is null then
    insert into public.properties (
      ...
      upper(p_payload ->> 'reference'),
      p_payload ->> 'slug',
      p_payload ->> 'title',
      (p_payload ->> 'source')::public.property_source,
      p_payload ->> 'propertyType',
      'draft',
      p_payload ->> 'locationName',
...
create function public.transition_property_status(
  p_property_id uuid,
...
revoke all on function public.save_property(jsonb) from public, anon;
revoke all on function public.transition_property_status(uuid, public.property_status)
from public, anon;
grant execute on function public.save_property(jsonb) to authenticated;
grant execute on function public.transition_property_status(uuid, public.property_status)
to authenticated;
