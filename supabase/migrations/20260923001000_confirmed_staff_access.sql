...
begin
  ...
  if p_active and (previous_row.user_id is null or not previous_row.active)
    and not exists (
      select 1 from auth.users where id = p_user_id
        and email_confirmed_at is not null
        and nullif(encrypted_password, '') is not null
    ) then
    raise check_violation using message = 'staff requires confirmed email and password';
  end if;
